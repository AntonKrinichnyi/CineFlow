...
def certification(db_session):
    certification = CertificationModel(name="TV-14")
...
@pytest.fixture(scope="function")
def star(db_session):
    star = StarModel(name="Linda Hamilton")
    db_session.add(star)
    db_session.commit()
...
def director(db_session):
    director = DirectorModel(name="James Cameron")
...
def test_create_movie(db_session):
    ...
    saved_movie = db_session.query(MovieModel).filter_by(name="The Terminator").first()
